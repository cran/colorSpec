...
actinometric.colorSpec <- function( x, multiplier=1, warn=FALSE )
    ...
    if( type == "light" )
        ...
        phys    = (h*c)/e
        final   = multiplier * phys
        log.string( INFO, "final multiplier = multiplier * ((h*c)/e) / lambda = %g * %g / lambda = %g / lambda.",
                            multiplier, phys, final )
        vec     = final / lambda
    ...
    if( warn )
        {
        log.string( WARN, "Object x has been converted from radiometric (quantity='%s') to actinometric (quantity='%s').",
