...
\code{emulate()} and \code{plotPatchesRGB()}.
...
# in cbind() use as.data.frame.model.matrix() so RGB is a single column 
df.ref = cbind( extradata(MacbethCC), as.data.frame.model.matrix(RGB.ref) )
...
df.foveon = cbind( extradata(MacbethCC), as.data.frame.model.matrix(RGB.foveon) )
...
RGB.foveon.mod = product( illum, MacbethCC, foveon.mod )
df.foveon.mod = cbind( extradata(MacbethCC), as.data.frame.model.matrix(RGB.foveon.mod) )
...
\code{emulate()} in future versions,
...
This option might be added to \code{emulate()} in the future.
...
# in cbind() use as.data.frame.model.matrix() so RGB.plumbicon is a single column 
df.plumbicon = cbind( as.data.frame.model.matrix(RGB.plumbicon), extradata(MacbethCC) )
par( omi=c(0,0,0,0), mai=c(0,0,0,0) )
...
RGB.dragon = matrix( graylevel, length(graylevel), 3 )
df.dragon = cbind( as.data.frame.model.matrix(RGB.dragon), extradata(MacbethCC) )
