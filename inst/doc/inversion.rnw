...
in this package, computes the \emph{centroid} of the set of all the metamers (if any).
...
The centroid method also works for estimating light sources,
...
@
These estimates are quite poor.  
...
does not work at all; for more on this see Appendix A and \cite{Davis2018} Section 13.
...
\section*{Appendix A - Inverse Colorimetry}
<<appendixRI, child='right-inverse.txt'>>=
@


\section*{Appendix B - The Macbeth ColorChecker\texttrademark}
