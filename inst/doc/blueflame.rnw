...
library( colorSpec )
@
Featured functions in this vignette are: \code{chop()}, \code{bind()},
and \code{plotPatchesRGB()}.
...
For Adobe RGB, the "CH part of blue flame" is still too vivid (the green is negative)
but the other 3 colors are accurate.
Here is a diagram comparing the 2 color spaces, taken from \cite{wiki:3}

\begin{center}
\includegraphics[width=0.8\textwidth]{CIExy1931_AdobeRGB_vs_sRGB}

Figure 9. A comparison of the Adobe RGB (1998) and sRGB chromaticity gamuts within the CIE 1931 xy chromaticity diagram. 
